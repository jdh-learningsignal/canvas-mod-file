  ...
  def successful_login(user, pseudonym, otp_passed = false)
    ...
    respond_to do |format|
      if (oauth = session[:oauth2])
        ...
        ##### custom: 로그인 첫 화면 직접 수정 시작 #####
        format.html { redirect_to delegated_auth_redirect_uri(redirect_back_or_default("https://cloud2.canvas.lineedu.kr/accounts/1/external_tools/24?launch_type=global_navigation") }
        ##### custom: 로그인 첫 화면 직접 수정 끝 #####
      end
